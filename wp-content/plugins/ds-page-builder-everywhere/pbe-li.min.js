...
if (window.tinymce)(function() {
    ...
    tinymce._ds_pbe_init =
        tinymce.init;
    tinymce.init = function(arg1) {
        if (!window.tinyMCEPreInit) {
            if (arg1.content_css) arg1.content_css += "," + ds_pbe_fb_config.editorCssUrl;
                        ...
                        for (var i = 1; i < shortcodeParts.length; ++i)
                            if (shortcodeParts[i].trim().substr(0, 13) == "global_module") {
                                ...
                                if (equalsPos != -1) {
                                    ...
                                    if ((value[0] == '"' || value[0] == "'") && value[0] == value[value.length - 1]) value = value.substr(1, value.length - 2);
                                    window.open((window.ds_pbe_fb_config ? ds_pbe_fb_config.editLayoutUrl : "?action=edit&post=") + value)
                                }
                            }
                }
            });
            if (oldSetup) return oldSetup(setupArg1)
        };
